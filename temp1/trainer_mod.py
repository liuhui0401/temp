...
class hdencode_Trainer(nn.Module):
    def __init__(self, hyperparameters):
        super(hdencode_Trainer, self).__init__()
...
#        self.dis_2B = MsImageDis(hyperparameters['input_dim_a'], hyperparameters['dis'])  # discriminator 2 for domain B
    ...
    def forward(self, x_a, x_b):
        ...
        c_5, s_5 = self.gen_BA.encode(x_b)
        half_size = 1
        ...
        self.x_B2_fake = self.gen_AB.decode(c_5, z_3)

        self.X_A_A1_pair = torch.cat((x_a, self.x_A_fake), -3)
        self.X_A_A2_pair = torch.cat((x_a, self.x_A2_fake), -3)
        # second cross translation
        self.z_content_recon_b, self.z_content_recon_a = self.genAB.forward(self.x_A2_fake, self.x_B2_fake)
        self.fake_A_recon = self.genAB.forward(self.z_content_recon_a, s_2)
        self.fake_B_recon = self.genAB.forward(self.z_content_recon_b, s_4)
...
#            x_B_recon = self.focus_translation(x_B_recon, x_b, x_B_recon_focus)
        ...
        self.loss_G_L1_A = self.criterionL1(self.fake_A_recon, self.real_A_encoded) * 10
        self.loss_G_L1_B = self.criterionL1(self.fake_B_recon, self.real_B_encoded) * 10
